...
class AssociadoControllerIntegrationTest extends Assertions {
	...
	class ExcluirAssociado {
		Associado mock;
		MessageStreaming<Map<String, Object>> mockMessage;
		...
		void setup() {
			mock = AssociadoMockTest.getAssociado();
			mockMessage = new MessageStreaming<>();
			Mockito.doReturn(mock).when(service).consultarPorCpfCnpj(Mockito.anyString());
			Mockito.doNothing().when(service).excluir(Mockito.anyString());
		}

		@Test
		@DisplayName("deve excluir um associado e emitir um evento pelo message broker")
		void sucesso_excluir_associado_test() throws Exception {
			mockMessage.setData(Collections.singletonMap("ok", true));
			Mockito.doReturn(mockMessage).when(publisher).exchange(Mockito.anyString(), Mockito.any(AssociadoEventDTO.class));

			mvc.perform(toRequest(mock))
				.andExpectAll(
					status().isOk(),
					jsonPath("ok", Matchers.equalTo(true))
				);
		}

		@Test
		@DisplayName("deve retornar 404 quando houver boletos pendentes para pagamento relacionados ao associado")
		void error_excluir_associado_test() throws Exception {
			mockMessage.setData(Collections.singletonMap("ok", false));
			Mockito.doReturn(mockMessage).when(publisher).exchange(Mockito.anyString(), Mockito.any(AssociadoEventDTO.class));

			mvc.perform(toRequest(mock))
				.andExpectAll(
					status().isBadRequest(),
					jsonPath("message", Matchers.equalToIgnoringCase("Exclusão não pode ser realizada, boletos pendentes!"))
				);
		}
	}
